...
<c:url var="addUrl" value="/ctl/Leave"></c:url>
	...
	<div class="col-sm-10 col-sm-offset-1 col-md-6 col-md-offset-3">
		<div class="panel panel-default ors-form-pannel">
			<div class="panel-heading">
				<c:if test="${form.id ==0}">
					<s:message code="label.addLeave" />
				</c:if>
				<c:if test="${form.id>0}">
					<s:message code="label.updateLeave" />
				</c:if>
			</div>
			<div class="panel-body">
				<%@include file="BusinessMessage.jsp"%>

				<sf:form action="${addUrl}" method="post" commandName="form" cssClass="form-horizontal">
					<sf:hidden path="id" />
					<s:bind path="name">
						<div
					...
					<s:bind path="toDate">
						<div
							class="form-group has-feedback ${status.error ? 'has-error' : ''}">
							<sf:label path="toDate" cssClass="col-sm-3">
								<s:message code="label.toDate" />
							</sf:label>

							<div class="col-sm-9" data-datepicker date-format="MM/dd/yyyy"
								date-typer="true" >
								<sf:input path="toDate" class="form-control"  />
								<sf:errors path="toDate" cssClass="help-block" />
							</div>
						</div>
					</s:bind>
					
					<s:bind path="fromDate">
						<div
							class="form-group has-feedback ${status.error ? 'has-error' : ''}">
							<sf:label path="fromDate" cssClass="col-sm-3">
								<s:message code="label.fromDate" />
							</sf:label>
							<div class="col-sm-9" data-datepicker date-format="MM/dd/yyyy"
								date-typer="true" >
								<sf:input path="fromDate" class="form-control"  />
								<sf:errors path="fromDate" cssClass="help-block" />
							</div>
						</div>
					</s:bind>
