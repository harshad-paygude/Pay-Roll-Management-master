...
<c:url var="addUrl" value="/ctl/TimeSheet" />
<c:url var="addSearch" value="/ctl/TimeSheet/Search" />
<c:url var="editUrl" value="/ctl/TimeSheet?id=" />

<div class="row">
	<div class="col-sm-12 col-sm-offset-0 col-md-12 col-md-offset-0">
		<div class="panel panel-default ors-form-pannel">
			<div class="panel-heading">
				<s:message code="label.listTimeSheet" />
			</div>
				...
				<sf:form action="${addSearch }" method="post"
					...
					<table id="listTable"
						...
						<thead>
							<tr>
								...
								<th><s:message code="label.hours" /></th>
								<th><s:message code="label.description" /></th>
								<th><s:message code="label.edit" /></th>
							</tr>
						</thead>
						<tbody>
							<c:forEach items="${list}" var="time" varStatus="ct">
								<tr>
									<td><input type="checkbox" name="ids" class="case" value="${time.id}"></td>
									<td><c:out value="${time.name}" /></td>
									<td><c:out value="${time.emailId}" /></td>
									<td><fmt:formatDate pattern="MM/dd/yyyy"
											value="${time.tDate}" /></td>
									<td><c:out value="${time.hours}" /></td>
									<td><c:out value="${time.description}" /></td>
									<td><a href="${editUrl} ${time.id}"><span
					...
					<ul class="pager">
						...
						<li><input type="button" class="btn btn-info"
							onclick="location.href='<c:url value="/ctl/TimeSheet" />';"
