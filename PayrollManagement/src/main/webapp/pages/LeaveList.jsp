...
<c:url var="addUrl" value="/ctl/Leave" />
<c:url var="addSearch" value="/ctl/Leave/Search" />
<c:url var="editUrl" value="/ctl/Leave?id=" />

<div class="row">
	<div class="col-sm-12 col-sm-offset-0 col-md-12 col-md-offset-0">
		<div class="panel panel-default ors-form-pannel">
			<div class="panel-heading">
				<s:message code="label.listLeave" />
			</div>
				...
				<sf:form action="${addSearch }" method="post"
					...
					<table id="listTable"
						...
						<thead>
							<tr>
								...
								<th><s:message code="label.toDate" /></th>
								<th><s:message code="label.fromDate" /></th>
								<th><s:message code="label.description" /></th>
								<th><s:message code="label.edit" /></th>
							</tr>
						</thead>
						<tbody>
							<c:forEach items="${list}" var="lev" varStatus="ct">
								<tr>
									<td><input type="checkbox" name="ids" class="case" value="${lev.id}"></td>
									<td><c:out value="${lev.name}" /></td>
									<td><fmt:formatDate pattern="MM/dd/yyyy"
											value="${lev.toDate}" /></td>
									<td><fmt:formatDate pattern="MM/dd/yyyy"
											value="${lev.fromDate}" /></td>
									<td><c:out value="${lev.description}" /></td>
									<td><a href="${editUrl} ${lev.id}"><span
					...
					<ul class="pager">
						...
						<li><input type="button" class="btn btn-info"
							onclick="location.href='<c:url value="/ctl/Leave" />';"
