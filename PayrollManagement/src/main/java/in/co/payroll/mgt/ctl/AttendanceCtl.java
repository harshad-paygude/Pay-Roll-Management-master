...
import in.co.payroll.mgt.form.AttendanceForm;
import in.co.payroll.mgt.service.AttendanceServiceInt;
...
public class AttendanceCtl extends BaseCtl {
	...
	@Autowired
	AttendanceServiceInt attendanceServiceInt;
	...
	public String submit(Locale locale, @RequestParam String operation,
			@ModelAttribute("form") @Valid AttendanceForm form, BindingResult bindingResult, Model model) {
	...
	public String searchList(Locale locale, @ModelAttribute("form") AttendanceForm form,
		...
		if (OP_DELETE.equals(operation)) {
			...
			if (form.getIds() != null) {
				...
				for (long id : form.getIds()) {
					...
					dto.setId(id);
					attendanceServiceInt.delete(dto);
				}
		...
		List<AttendanceDTO> list = attendanceServiceInt.search(dto, pageNo, pageSize);

		List<AttendanceDTO> totallist = attendanceServiceInt.search(null);
